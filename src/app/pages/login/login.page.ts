    ...
    this.db.loadUser(this.username).then(usr => { 
      ...
      if (success == true) { 
        if(fetch == true) {
          ...
          //this.db.deleteAllData().then(() => {
            //this.db.Fetchdata(this.username, save_login, usr.agent_type, usr.id_contact, usr.id_primary_company, usr.id_supchain_company);
          //});

        } else {
          this.db.logIn(this.username, save_login).then(() => {
            //this.db.updateRegvalues();
            //this.db.backup(usr.id_contact);
